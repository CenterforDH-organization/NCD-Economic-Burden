...
### 3. Imputation (결과 보정)
```bash
python imputation.py -i results/aggregate_results.csv -o tmpresults/aggregate_results_imputed.csv
```
- 음수 값이나 누락된 값을 통계적 모델로 보정합니다

### 4. 테이블 생성
```bash
python generate_tables.py -f tmpresults/aggregate_results_imputed.csv -d 0.02 -i 0.11
```
...
rm -f tmpresults/aggregate_results_imputed.csv
